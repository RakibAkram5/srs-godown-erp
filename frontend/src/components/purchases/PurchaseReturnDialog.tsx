...
export function PurchaseReturnDialog({ purchase, open, onOpenChange, currency = 'PKR' }: Props) {
  ...
  const total = useMemo(
    () => items.reduce((s, it) => s + (qty[it.productId] || 0) * it.purchasePrice, 0),
    [items, qty],
  );
    ...
    mutationFn: () => {
      const returnItems = items
        ...
        .map((it) => ({
          ...
          quantity: qty[it.productId],
          price: it.purchasePrice,
        }));
      return purchasesApi.createReturn({ purchaseId: purchase!.id, notes: notes || null, items: returnItems });
    },
    onSuccess: () => {
  ...
  function submit() {
    ...
    for (const it of items) {
      const q = qty[it.productId] || 0;
      if (q > it.quantity) return toast.error('Too many', `Cannot return more than ${it.quantity} of ${it.productName}.`);
    }
    mutation.mutate();
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Return items — {purchase?.purchaseNo}</DialogTitle>
          <DialogDescription>Enter the quantity to return for each product. Stock will decrease.</DialogDescription>
        </DialogHeader>
                ...
                <TableRow key={it.productId}>
                  ...
                  <TableCell>
                    <Input
                      ...
                      onChange={(e) =>
                        setQty((prev) => ({ ...prev, [it.productId]: Math.max(0, Math.min(it.quantity, Number(e.target.value))) }))
                      }
        ...
        <div className="space-y-2">
          ...
          <Textarea id="rnotes" rows={2} placeholder="Optional" value={notes} onChange={(e) => setNotes(e.target.value)} />
        </div>
